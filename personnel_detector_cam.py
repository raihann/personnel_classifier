...
from per_classifier import vest_classifier
from per_classifier import box_filter
from simple_cb import simplest_cb as cb
...
#    filtered_boxes, filtered_scores, filtered_classes = filter_boxes(50.0, np.squeeze(boxes), np.squeeze(scores), np.squeeze(classes), categories)
    ...
    image_np = cb(image_np, 10)

    display_image, red_mask, green_mask = vest_classifier(image_np, filtered_boxes, 320, 240)
...
#    video_capture = cv2.VideoCapture('cor1.mpg')
    fps = FPS().start()
    ...
    while True:  # fps._numFrames < 120
        ...
        cv2.rectangle(output_rgb, (220, 0), (350, 20), (255, 0, 0), -1)
        cv2.putText(output_rgb, str(p_count), (240, 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
