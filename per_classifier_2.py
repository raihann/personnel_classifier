...
def create_mask(source_image, lower_bound, upper_bound):
    ...
    hsv_img = cv2.cvtColor(source_image, cv2.COLOR_RGB2HSV)
...
def vest_classifier(display_image, boxes, width, height):
    ...
    red_thresh = 50
    red_center = np.array([199, 195, 173])
    #red_lower_bound = np.array([red_center[0]-red_thresh, red_center[1]-red_thresh, red_center[2]-red_thresh])
    #red_upper_bound = np.array([red_center[0]+red_thresh, red_center[1]+red_thresh, red_center[2]+red_thresh])

    # set bounds for color ranges for red and green mask in HSV color space
    # these values will be moved to a configuration file in the next version
    red_lower_bound = np.array([0, 152, 130])
    red_upper_bound = np.array([17, 255, 255])
    green_lower_bound = np.array([25, 128, 70])
    green_upper_bound = np.array([40, 255, 255])

    red_mask, hsv_img = create_mask(display_image, red_lower_bound, red_upper_bound)
    green_mask, _ = create_mask(display_image, green_lower_bound, green_upper_bound)
    red_result = cv2.bitwise_and(hsv_img, hsv_img, mask=red_mask)
    _, red_conts, h = cv2.findContours(cv2.cvtColor(red_result.copy(), cv2.COLOR_BGR2GRAY), cv2.RETR_EXTERNAL, \
    ...
    for i,r in enumerate(red_rects):
        for j,b in enumerate(boxes):
            ...
            (b[2]*height > r[1]+r[3])):
                ...
                (int(b[3]*width), int(b[2]*height)), (255,0,0), 2)
    ...
    return display_image, green_filtered_conts, red_filtered_conts        
